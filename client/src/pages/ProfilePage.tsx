...
export const ProfilePage = () => {
  ...
  return (
    <div className="mx-auto max-w-lg space-y-5 pb-10">
      <h1 className="text-2xl font-bold tracking-tight text-stone-800">{t.profile}</h1>

      {/* Profile header */}
      <div className="relative overflow-hidden rounded-2xl bg-white ring-1 ring-stone-200/60">
      ...
      {activePlan && (
        <div className="rounded-2xl bg-white p-5 ring-1 ring-stone-200/60">
          ...
          <div className="mt-3 flex items-end justify-between">
            ...
            <span className="text-sm font-bold tabular-nums text-stone-800">{progressPercent}%</span>
          </div>
      ...
      <div className="pt-2">
        <button
          onClick={logout}
          className="flex h-11 w-full items-center justify-center rounded-xl text-sm font-medium text-stone-400 transition-colors hover:bg-stone-100 hover:text-stone-600 active:bg-stone-200"
        >
          {t.logout}
        </button>
      </div>
...
const StatCard = ({ value, label, isHighlight }: StatCardProps) => (
  <div className="rounded-2xl bg-white p-4 ring-1 ring-stone-200/60">
    <p className={`text-2xl font-bold tabular-nums ${isHighlight ? 'text-amber-600' : 'text-stone-800'}`}>
      {value}
    </p>
