...
// GET all journals for user (paginated, newest first)
...
// POST create or update journal for a date
router.post('/', async (req, res, next) => {
  try {
    ...
    const validVerses = Array.isArray(linkedVerses)
      ? linkedVerses.filter((v) => v.bookAbbr && v.bookName && v.chapter && v.verse).slice(0, 10)
      : [];
